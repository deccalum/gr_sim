#pragma once

/**
 * @brief Stub gravitational-wave emission engine reserved for future radiation reaction work.
 */

#include "engine_interface.h"

class SimulationState;

/**
 * @brief Placeholder gravitational-wave emission plugin.
 */
class gw_emission_engine final : public EngineInterface {
 public:
  std::string name() const override { return "gw_emission"; }
  std::string version() const override { return "0.0.0-stub"; }
  void init(SimulationState&) {}
  void pre_step(SimulationState&, double) {}
  void post_step(SimulationState&, double) {}
  void shutdown() {}
  std::vector<std::string> reads() const override { return {}; }
  std::vector<std::string> writes() const override { return {}; }
};
